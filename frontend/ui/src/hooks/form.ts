...
import { useErrorState, useLoadingState } from './common'

/**
 * 表单字段配置接口
 */
export interface FormFieldConfig<TValue = unknown> {
  required?: boolean
      ...
      try {
        ...
        Object.keys(values).forEach((key) => {
          ...
          if (fieldConfig?.transform) {
            transformedValues[fieldKey] = fieldConfig.transform(
              values[fieldKey],
            ) as T[keyof T]
          }
        })
 ...
 * 针对项目中大量对话框表单的使用场景
 */
export const useDialogForm = <T extends Record<string, unknown>>(
  initialValues: T,
  options: {
    onSubmit?: (values: T) => void | Promise<void>
    onSuccess?: () => void
